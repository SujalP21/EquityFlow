...
const { PriceHistoryModel } = require("../model/PriceHistoryModel");
...
stockRoutes.get("/stocks/search", async (req, res) => {
  try {
...
stockRoutes.get("/stocks/sectors", async (req, res) => {
  try {
...
stockRoutes.get("/stocks/:symbol/history", async (req, res) => {
  try {
...
stockRoutes.get("/stocks/:symbol", async (req, res) => {
  try {
    const symbol = String(req.params.symbol || "").trim().toUpperCase();
    const stock = await StockModel.findOne({ symbol, active: true }).lean();
...
stockRoutes.get("/stocks", async (req, res) => {
  try {
